...
extern unsigned char* hataroid_getAssetDataRef(JNIEnv *curEnv, const char* assetPath, int persist, int *len, int *id);

DSFILE* dsFile_fopen(const char *filename, const char *mode)
{
	DSFILE* f = 0;
	if (filename != 0)
	{
		...
		{
			FILE *fp = FLUID_FOPEN(filename, mode);
			if (fp != 0)
			{
				f = FLUID_NEW(DSFILE);
				memset(f, 0, sizeof(DSFILE));
	...
	return f; // be careful with this return struct
}

int dsFile_fclose(DSFILE* fd)
{
	int res = 0;
	if (fd)
	{
		if (fd->mem_buf_id)
		{
			hataroid_releaseAssetDataRef(fd->mem_buf_id);
			fd->mem_buf_id = 0;
		}
		else if (fd->f)
		{
			res = FLUID_FCLOSE(fd->f);
			fd->f = 0;
		}
...
int dsFile_fseek(DSFILE* fd, long int offset, int origin)
{
	if (fd)
	{
		if (fd->mem_buf_id)
		{
			int newOffset = 0;
			if (origin == SEEK_SET)			{ newOffset = offset; }
			else if (origin == SEEK_CUR)	{ newOffset = fd->mem_curOffset + offset; }
			else if (origin == SEEK_END)	{ newOffset = fd->mem_size + offset; }

			if (newOffset < 0)					{ /*fd->mem_curOffset = 0;*/ return -1; }
			//else if (newOffset == fd->mem_size)	{ fd->mem_curOffset = fd->mem_size; /*fd->mem_eof = 1;*/ return 0; }
			else if (newOffset > fd->mem_size)	{ /*fd->mem_curOffset = fd->mem_size;*/ /*fd->mem_eof = 1;*/ return -1; }

			fd->mem_eof = 0;
			fd->mem_curOffset = newOffset;
			return 0;
		}
		else if (fd->f)
		{
			return FLUID_FSEEK(fd->f, offset, origin);
		}
	}
...
size_t dsFile_fread(void* ptr, size_t size, size_t count, DSFILE* fd)
{
	if (fd)
	{
		if (fd->mem_buf_id)
		{
			...
			if (fd->mem_curOffset >= fd->mem_size)	{ fd->mem_eof = 1; return 0; }

			int bufAvail = fd->mem_size - fd->mem_curOffset;
			int countAvail = bufAvail / size;
			int readCount = 0;
			if (countAvail > 0)
			{
				readCount = (countAvail > count) ? count : countAvail;
				int rsize = size*readCount;
				memcpy(ptr, &fd->mem_buf[fd->mem_curOffset], rsize);
				fd->mem_curOffset += rsize;
			}
			if (countAvail < count || fd->mem_curOffset >= fd->mem_size) { fd->mem_eof = 1; }
			return readCount;
		}
		else if (fd->f)
		{
			return FLUID_FREAD(ptr, size, count, fd->f);
		}
	}
	return 0;
}

int dsFile_feof(DSFILE* fd)
{
	if (fd)
	{
		if (fd->mem_buf_id)
...
long int dsFile_ftell(DSFILE* fd)
{
	if (fd)
	{
		if (fd->mem_buf_id)
		{
			return fd->mem_curOffset;
		}
		else if (fd->f)
		{
			return ftell(fd->f);
...
fluid_defsfont_load_sampledata(fluid_defsfont_t* sfont)
{
  ...
  DSFILE* fd = dsFile_fopen(sfont->filename, "rb");
  if (!fd) {
    FLUID_LOG(FLUID_ERR, "Can't open soundfont file");
    return FLUID_FAILED;
  }
  if (dsFile_fseek(fd, sfont->samplepos, SEEK_SET) == -1) {
    perror("error");
  ...
  if (dsFile_fread(sfont->sampledata, 1, sfont->samplesize, fd) < sfont->samplesize) {
...
static int load_body (unsigned int size, SFData * sf, DSFILE * fd);
static int read_listchunk (SFChunk * chunk, DSFILE * fd);
static int process_info (int size, SFData * sf, DSFILE * fd);
static int process_sdta (unsigned int size, SFData * sf, DSFILE * fd);
static int pdtahelper (unsigned int expid, unsigned int reclen, SFChunk * chunk,
  int * size, DSFILE * fd);
static int process_pdta (int size, SFData * sf, DSFILE * fd);
static int load_phdr (int size, SFData * sf, DSFILE * fd);
static int load_pbag (int size, SFData * sf, DSFILE * fd);
static int load_pmod (int size, SFData * sf, DSFILE * fd);
static int load_pgen (int size, SFData * sf, DSFILE * fd);
static int load_ihdr (int size, SFData * sf, DSFILE * fd);
static int load_ibag (int size, SFData * sf, DSFILE * fd);
static int load_imod (int size, SFData * sf, DSFILE * fd);
static int load_igen (int size, SFData * sf, DSFILE * fd);
static int load_shdr (unsigned int size, SFData * sf, DSFILE * fd);
...
sfload_file (const char * fname)
{
  ...
  DSFILE* fd = dsFile_fopen(fname, "rb");
  if (!fd)
    {
  ...
  /* get size of file */
  if (!err && dsFile_fseek (sf->sffd, 0L, SEEK_END) == -1)
    {				/* seek to end of file */
      err = TRUE;
      FLUID_LOG (FLUID_ERR, _("Seek to end of file failed"));
    }
  if (!err && (fsize = dsFile_ftell (sf->sffd)) == -1)
    {				/* position = size */
      err = TRUE;
      FLUID_LOG (FLUID_ERR, _("Get end of file position failed"));
    }
  if (!err)
    dsFile_rewind (sf->sffd);

  if (!err && !load_body (fsize, sf, sf->sffd))
...
static int
load_body (unsigned int size, SFData * sf, DSFILE * fd)
{
  SFChunk chunk;
...
static int
read_listchunk (SFChunk * chunk, DSFILE * fd)
{
...
static int
process_info (int size, SFData * sf, DSFILE * fd)
{
  SFChunk chunk;
  unsigned char id;
  char *item;
...
static int
process_sdta (unsigned int size, SFData * sf, DSFILE * fd)
{
  SFChunk chunk;
...
static int
process_pdta (int size, SFData * sf, DSFILE * fd)
{
  SFChunk chunk;
...
static int
load_phdr (int size, SFData * sf, DSFILE * fd)
{
  int i, i2;
...
/* preset bag loader */
static int
load_pbag (int size, SFData * sf, DSFILE * fd)
{
  fluid_list_t *p, *p2;
  SFZone *z, *pz = NULL;
...
static int
load_pmod (int size, SFData * sf, DSFILE * fd)
{
 ...
 * ------------------------------------------------------------------- */
static int
load_pgen (int size, SFData * sf, DSFILE * fd)
{
...
static int
load_ihdr (int size, SFData * sf, DSFILE * fd)
{
  int i, i2;
...
static int
load_ibag (int size, SFData * sf, DSFILE * fd)
{
  fluid_list_t *p, *p2;
  SFZone *z, *pz = NULL;
...
static int
load_imod (int size, SFData * sf, DSFILE * fd)
{
  fluid_list_t *p, *p2, *p3;
  SFMod *m;
...
static int
load_igen (int size, SFData * sf, DSFILE * fd)
{
  fluid_list_t *p, *p2, *p3, *dup, **hz = NULL;
  SFZone *z;
  SFGen *g;
...
static int
load_shdr (unsigned int size, SFData * sf, DSFILE * fd)
{
  unsigned int i;
  SFSample *p;
...
int
safe_fread (void *buf, int count, DSFILE * fd)
{
  if (dsFile_fread (buf, count, 1, fd) != 1)
...
int
safe_fseek (DSFILE * fd, long ofs, int whence)
{
  if (dsFile_fseek (fd, ofs, whence) == -1) {
