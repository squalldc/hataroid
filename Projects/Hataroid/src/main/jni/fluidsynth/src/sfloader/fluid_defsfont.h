...
// HACK --- to allow read from memory buffer
typedef struct
{
	FILE* f;
...
{				/* Sound font data structure */
  ...
  DSFILE *sffd;			/* loaded sfont file descriptor */
...
int safe_fread (void *buf, int count, DSFILE * fd);
int safe_fwrite (void *buf, int count, DSFILE * fd);
int safe_fseek (DSFILE * fd, long ofs, int whence);
