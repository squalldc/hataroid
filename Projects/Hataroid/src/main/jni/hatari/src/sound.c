...
static void Sound_GenerateSamples(int SamplesToGenerate)
{
	...
	// generate midi audio
	{
		int reqLenShorts = SamplesToGenerate << 1;

		fsMidi_generateFrames(SamplesToGenerate);

		int startOffsetShorts = 0;
		int readLenShorts = 0;
		int bufMaxShorts = 0;

		short *midiBuf = fsMidi_consumeBuffer(reqLenShorts, &startOffsetShorts, &readLenShorts, &bufMaxShorts);
		if (midiBuf != 0)
		{
			int maxSamples = readLenShorts >> 1;

			i = 0;
			int s = startOffsetShorts + (i<<1);
			short *curMidiBuf = &midiBuf[s];
			if (ConfigureParams.Hataroid.muteSTSound)
			{
				for (; i < maxSamples; ++i)
				{
					idx = (ActiveSndBufIdx + i) % MIXBUFFER_SIZE;

					MixBuffer[idx][0] = (*curMidiBuf); ++curMidiBuf;
					MixBuffer[idx][1] = (*curMidiBuf); ++curMidiBuf;
				}
			}
			else
			{
