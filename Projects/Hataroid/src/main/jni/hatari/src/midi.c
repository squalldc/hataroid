...
//extern Uint32 nVBLCount;
//Uint32 nCheckFrame = 0;
//int midiIntCount = 0;
//int lastMidiIntCount = 0;
...
void Midi_InterruptHandler_Update(void)
{
	...
	/*if (nVBLCount != nCheckFrame)
	{
		if (lastMidiIntCount != midiIntCount)
		{
			Debug_Printf("Midi Intr Count: %d", midiIntCount);
			lastMidiIntCount = midiIntCount;
		}
		nCheckFrame = nVBLCount;
		midiIntCount = 0;
	}
	++midiIntCount;
	*/
	...
	//bool hasData = pMidiFhIn && File_InputAvailable(pMidiFhIn);
	int hasData = fsMidi_hasRecvData();

	if (hasData)
	{
		nInChar = fsMidi_readRecvData();
