...
const char FDC_Old_fileid[] = "Hatari fdc_old.c : " __DATE__ " " __TIME__;
 ...
 */
void FDC_MemorySnapShot_Capture_Old(bool bSave)
{
 ...
 */
void FDC_SetDMAStatus_Old ( bool bError )
{
	/* Set error bit */
	if (!bError)
 ...
 */
void FDC_AcknowledgeInterrupt_Old ( void )
{
 ...
 */
void FDC_InterruptHandler_Update_Old ( void )
{
	int	Delay_micro = 0;
...
static int FDC_TypeII_ReadSector ( void )
{
	...
	LOG_TRACE(TRACE_FDC, "fdc type II read sector sector=0x%x multi=%s spinup=%s settle=%s tr=0x%x head_track=0x%x side=%d drive=%d dmasector=%d addr=0x%x VBL=%d video_cyc=%d %d@%d pc=%x\n",
		  ...
		  FDC_GetDMAAddress_Old(), nVBLs, FrameCycles, LineCycles, HblCounterVideo, M68000_GetPC());
...
static int FDC_TypeIII_ReadAddress ( void )
{
	...
	LOG_TRACE(TRACE_FDC, "fdc type III read address spinup=%s settle=%s tr=0x%x head_track=0x%x side=%d drive=%d addr=0x%x VBL=%d video_cyc=%d %d@%d pc=%x\n",
		  ( FDC.CR & FDC_COMMAND_BIT_MOTOR_ON ) ? "off" : "on" ,
		  ( FDC.CR & FDC_COMMAND_BIT_HEAD_LOAD ) ? "on" : "off" ,
		  FDC.TR , HeadTrack[ FDC_DRIVE ], FDC_SIDE, FDC_DRIVE , FDC_GetDMAAddress_Old(),
		  nVBLs, FrameCycles, LineCycles, HblCounterVideo, M68000_GetPC());
 ...
 */
void FDC_DiskController_WriteWord_Old ( void )
{
	int FrameCycles, HblCounterVideo, LineCycles;
	...
	if ( ( FDC_DMA.Mode & 0x0018 ) == 8 )
	{
		...
		HDC_WriteCommandByte(FDC_DMA.Mode & 0x7, IoMem_ReadByte(0xff8605)); //HDC_WriteCommandPacket();
		return;
	}
 ...
 */
void FDC_DiskControllerStatus_ReadWord_Old ( void )
{
	...
	if ((FDC_DMA.Mode & 0x18) == 0x08)				/* HDC status reg selected? */
	{
        ...
        DiskControllerByte = HDC_ReadCommandByte(FDC_DMA.Mode & 0x7); //DiskControllerByte = HDC_GetCommandStatus();
	}
	else if ((FDC_DMA.Mode & 0x18) == 0x18)				/* HDC sector counter??? */
	{
		Log_Printf(LOG_DEBUG, "*** Read HDC sector counter???\n");
        DiskControllerByte = 0; //DiskControllerByte = HDC_GetSectorCount(); // TODO?
	}
	else
	{
		/* FDC code */
 ...
 */
void FDC_DmaModeControl_WriteWord_Old ( void )
{
 ...
 */
void FDC_DmaStatus_ReadWord_Old ( void )
{
	if (nIoMemAccessSize == SIZE_BYTE)
	{
		/* This register does not like to be accessed in byte mode on a normal ST */
	...
	if ( FDC_DMA.Mode & 0x08 )					/* Get which sector count ? */
		FDC_DMA.Status |= 0; // TODO? (HDC_GetSectorCount())?0x2:0;		/* HDC */
	else
 ...
 */
void	FDC_DmaAddress_ReadByte_Old ( void )
{
	int FrameCycles, HblCounterVideo, LineCycles;
	...
	LOG_TRACE(TRACE_FDC, "fdc read dma address %x val=0x%02x address=0x%x VBL=%d video_cyc=%d %d@%d pc=%x\n" ,
		IoAccessCurrentAddress , IoMem[ IoAccessCurrentAddress ] , FDC_GetDMAAddress_Old() ,
		nVBLs , FrameCycles, LineCycles, HblCounterVideo , M68000_GetPC() );
}
 ...
 */
void	FDC_DmaAddress_WriteByte_Old ( void )
{
	int FrameCycles, HblCounterVideo, LineCycles;
 ...
 */
Uint32 FDC_GetDMAAddress_Old(void)
{
	Uint32 Address;
 ...
 */
void FDC_WriteDMAAddress_Old ( Uint32 Address )
{
	int FrameCycles, HblCounterVideo, LineCycles;
...
static bool FDC_ReadSectorFromFloppy ( Uint8 *buf , Uint8 Sector , int *pSectorSize )
{
	...
	LOG_TRACE(TRACE_FDC, "fdc read sector addr=0x%x dev=%d sect=%d track=%d side=%d VBL=%d video_cyc=%d %d@%d pc=%x\n" ,
		FDC_GetDMAAddress_Old(), FDC_DRIVE, Sector, HeadTrack[ FDC_DRIVE ], FDC_SIDE,
		nVBLs , FrameCycles, LineCycles, HblCounterVideo , M68000_GetPC() );

	/* Copy 1 sector to our workspace */
    Uint8 *readBuf = 0;
	if ( Floppy_ReadSectors ( FDC_DRIVE, &readBuf, Sector, HeadTrack[ FDC_DRIVE ], FDC_SIDE, 1, NULL, pSectorSize ) )
    {
        memcpy(buf, readBuf, (*pSectorSize));
		return true;
    }
...
static bool FDC_WriteSectorToFloppy ( int DMASectorsCount , Uint8 Sector , int *pSectorSize )
{
	...
	if ( DMASectorsCount > 0 )
		pBuffer = &STRam[ FDC_GetDMAAddress_Old() ];
	else
	{
 ...
 */
void FDC_FloppyMode_WriteByte_Old ( void )
{
 ...
 *           +-------- Disk changed
 */
void FDC_FloppyMode_ReadByte_Old ( void )
{
...
void FDC_InsertFloppy_Old ( int Drive ) { }
void FDC_EjectFloppy_Old ( int Drive ) { }
void FDC_SetDriveSide_Old ( Uint8 io_porta_old , Uint8 io_porta_new ) { }
int	FDC_GetBytesPerTrack_Old ( int Drive ) { return 0; }
int	FDC_IndexPulse_GetCurrentPos_FdcCycles_Old ( Uint32 *pFdcCyclesPerRev ) { return -1; }
int FDC_IndexPulse_GetCurrentPos_NbBytes_Old ( void ) { return 0; }
int FDC_IndexPulse_GetState_Old ( void ) { return 0; }
int FDC_NextIndexPulse_FdcCycles_Old ( void ) { return 0; }
Uint8 FDC_GetCmdType_Old ( Uint8 CR ) { return 0; }
void FDC_Buffer_Reset_Old ( void ) { }
void FDC_Buffer_Add_Timing_Old ( Uint8 Byte , Uint16 Timing ) { }
void FDC_Buffer_Add_Old ( Uint8 Byte ) { }
Uint16 FDC_Buffer_Read_Timing_Old ( void ) { return 0; }
Uint8 FDC_Buffer_Read_Byte_Old ( void ) { return 0; }
Uint8 FDC_Buffer_Read_Byte_pos_Old ( int pos ) { return 0; }
int	FDC_Buffer_Get_Size_Old ( void ) { return 0; }
void FDC_SetIRQ_Old ( Uint8 IRQ_Source ) { }
void FDC_ClearIRQ_Old ( void ) { }
void FDC_ClearHdcIRQ_Old (void) { }
void FDC_Drive_Set_BusyLed_Old ( Uint8 SR ) { }
int FDC_DMA_GetModeControl_R_WR_Old ( void ) { return 0; }
void FDC_DMA_FIFO_Push_Old ( Uint8 Byte ) { }
Uint8 FDC_DMA_FIFO_Pull_Old ( void ) { return 0; }
