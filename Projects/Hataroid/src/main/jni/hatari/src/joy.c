...
Uint8 Joy_GetStickData(int nStJoyId)
{
	...
	if (UseCustomJoyEmu)
	{
		nData = nJoyCustomEmu[nStJoyId];
	}
	/* Are we emulating the joystick via the keyboard? */
	else if (ConfigureParams.Joysticks.Joy[nStJoyId].nJoystickMode == JOYSTICK_KEYBOARD)
	{
...
static int Joy_GetFireButtons(int nStJoyId)
{
	...
	if (UseCustomJoyEmu)
	{
		nButtons |= (nJoyCustomEmu[nStJoyId] & ATARIJOY_BITMASK_FIRE) ? 1 : 0;
	}
	/* Are we emulating the joystick via the keyboard? */
	else if (ConfigureParams.Joysticks.Joy[nStJoyId].nJoystickMode == JOYSTICK_KEYBOARD)
	{
...
void Joy_SetCustomEmu(int set)
{
	if (set != UseCustomJoyEmu)
	{
		UseCustomJoyEmu = set;
		memset(nJoyCustomEmu, 0, sizeof(Uint8)*JOYSTICK_COUNT);
	}
}

bool Joy_CustomDown(int joy, int joyButtonSetFlag, int joyButtonClearFlag)
{
	if (joy < JOYSTICK_COUNT)
	{
		// just re-using the bit flags so we don't have to redefine another set of enums
		nJoyCustomEmu[joy] |= joyButtonSetFlag;
		nJoyCustomEmu[joy] &= ~joyButtonClearFlag;
		return true;
	}
	return false;
}

bool Joy_CustomUp(int joy, int joyButtonClearFlag)
{
	if (joy < JOYSTICK_COUNT)
	{
		// just re-using the bit flags so we don't have to redefine another set of enums
