...
Uint32 nVBLCount = 0;                  /* Frame count */
...
{
	int dx = pEvent->motion.xrel;
	int dy = pEvent->motion.yrel;
	hatari_onMouseMoved(dx, dy);
}

void hatari_onMouseMoved(int dx, int dy)
{
...
static void Main_Init(void)
{
	...
	{
		DlgAlert_Notice("Failed to load TOS image!\nPlease try choosing another file next time.\n\nQuitting!!");
		RequestAndWaitQuit();
...
extern void hataroid_loadInitialConfig();
 ...
 */
int hatari_main_init(int argc, const char *argv[])
{
	int ret;

	//ret = SDL_putenv("SDL_VIDEODRIVER=dummy");
